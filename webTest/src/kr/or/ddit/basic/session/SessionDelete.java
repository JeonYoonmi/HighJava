...
public class SessionDelete extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 2. session객체를 생성하거나 현재 session가져오기
		//		==> 세션 자체는 삭제되지 않고 개별적인 세션 값만 삭제 된다.
		//			(지정한 'key'값과 일치하는 세션값 삭제)
		session.removeAttribute("testSession");
		
		// 3. session객체.invalidate();	==> 세션 자체가 삭제된다.
		session.invalidate();
		...
		out.println("<body>");
		out.println("<h3>저장된 Session정보 삭제하기</h3><hr>");
