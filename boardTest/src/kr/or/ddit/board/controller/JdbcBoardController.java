...
public class JdbcBoardController {
	...
	public void boardStart() {
		String serchTitle = null;
		
		while(true) {
			int choice = displayMenu(serchTitle);
			serchTitle = null;
			switch(choice) {
			case 1 : // 새글 작성
				insertBoard(); break;
	...
	//개시판 목록을 보여주고 메뉴를 나타내며 사용자가 입력한 메뉴 번호를 반환하는 메서드
	private int displayMenu(String serchTitle) {
		System.out.println();
		System.out.println("-------------------------------------------------");
		System.out.println("No	        제 목            작성자 	조회수   ");
		System.out.println("-------------------------------------------------");
		List<JdbcBoardVO> boardList = null;
		if(serchTitle == null || "".equals(serchTitle)) {
			boardList = service.getAllBoardList();
		}else {
			boardList = service.getSearchBoardList(serchTitle);
		}
